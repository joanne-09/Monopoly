...
export default class MapManager extends cc.Component {
  ...
  onLoad() {
    if (MapManager.instance) {
      console.warn("MapManager instance already exists, destroying this node.");
      this.node.destroy();
      return;
    }
    ...
    if (!cc.game.isPersistRootNode(this.node)) {
      ...
      console.log("MapManager: Persist root node set.");
    }
    this.gameButton.node.on('click', this.loadGameScene, this);
  }

  onEnable() {
    console.log("MapManager: onEnable called. Refreshing visual state.");
    this.gameManager = GameManager.getInstance(); // Ensure reference is fresh

    if (this.gameManager && this.gameManager.getIsGameActive()) {
      this.refreshMapVisualState(); // You'll need to create this method
      this.updateButtonStates();    // You'll need to create this method
    } else {
      console.warn("MapManager: Game is not active, skipping refresh.");
    }
  }

  refreshMapVisualState() {
    // Update moneyLabel
    if (this.moneyLabel && this.gameManager) {
      const localPlayer = this.gameManager.getLocalPlayerData();
      if (localPlayer) {
        this.moneyLabel.string = `Money: ${localPlayer.money}`;
        this.lastMoney = localPlayer.money; // Update lastMoney if you use it for change detection
      }
    }
    // Re-draw player ownerships on spaces, etc.
    // Update other visual elements based on GameManager state
  }
  updateButtonStates() {
    if (this.gameButton) {
      // Set interactable based on game state, e.g.
      // this.gameButton.interactable = this.gameManager.canStartMinigame();
      this.gameButton.interactable = true; // Or based on some logic
    }
  }
  ...
  private loadGameScene() {
    console.log("MapManager: Loading game scene.");
